...
//mailer function
function introMailer($recipient, $subject, $body, $bcc = NULL) {
require get_template_directory() . '/class.phpmailer.php';
    ...
    $mail->From = 'fitnesscakenoreply@gmail.com';
    $mail->FromName = get_bloginfo('name');
    $mail->AddAddress($recipient);  
    $mail->AddBCC('tyler.smith.la@gmail.com');
...
?>
  ...
  <meta property="og:title" content="<?php echo $post->post_title. ' | '.$blog_details->blogname; ?>" />
  ...
  <meta property="fb:app_id" content="682740565144606" />
<?php
  }elseif (is_home()){
    ...
    ?>
  <meta property="og:url" content="<?php echo $blog_details->siteurl; ?>" />
  <meta property="og:title" content="<?php echo $blog_details->blogname; ?>" />
  <meta property="og:description" content="<?php echo $profile_txt[0]; ?>" />
  <meta property="og:image" content="<?php echo $profile_image[0]; ?>" />
  <meta property="fb:app_id" content="682740565144606" />
  <link rel="<?php echo $profile_image[0]; ?>" type="image/jpeg" href="<?php echo $profile_image[0]; ?>" /
<?php
...
//START EDITOR PROVISIONING
if (( current_user_can( 'edit_posts' )) && ( !current_user_can( 'activate_plugins' ))):

include(get_template_directory() . "/inc/editor_provisioning.php");

endif;  //END editor provisioning
...
function my_footer_version() {
    return 'Powered by Fitness Cake';
}
...
function routine_alert() {
  // post meta must be saved first..
      if( ( $_POST['post_status'] == 'publish' ) && ( $_POST['original_post_status'] != 'publish' ) ) {
        add_action('save_post','routine_alert_save',11,1);
      }
}
add_action('publish_routines_single','routine_alert');

function routine_alert_save($post_id) { 
  global $post, $blog_id, $routines, $routine_intended;
  $intended_meta = $routine_intended->the_meta($post_id); $intended = array();
   $current_blog_details = get_blog_details( array( 'blog_id' => $blog_id ) );

  $blogusers = get_users( 'blog_id='.$blog_id.'&role=author' );
   $intended_meta = $routine_intended->the_meta($post->ID); $intended = array(); 
   
        foreach ($intended_meta['client'] as $intended_client){ 
        if (in_array("all", $intended_client)){$intended[] = true;}
        }
    ...
    foreach ( $blogusers as $bloguser ) { 
      ...
      $notify_ok = get_user_meta($bloguser->ID, 'user_details');
      if($notify_ok[0] ["routine_alert"] == "1" && $intended[0]){
      ...
      //the select few
    foreach ($intended_meta['client'] as $intended_client){
      $user_meta_2 = get_userdata( $intended_client["target"]);
      $user_meta = get_user_meta( $intended_client["target"]);
      $subject = $current_blog_details->blogname . " Added a New Routine!";
      $body = "Hi ".$user_meta["first_name"][0].", <br/><br/>". $current_blog_details->blogname . " has added a new routine, &quot;". get_the_title($post->ID) ."&quot;. <br/><br/>Click the following link to check it out: ".get_permalink($post->ID)."<br/><br/>If you cannot click on the link just copy/paste it to your browser.<br/><br/>Thanks, <br/><br/>The Fitness Cake Team<br/><br/><font style='font-size:.8em;'>You can unsubscribe to these notifications on your Fitness Cake Settings page</font>";

      introMailer($user_meta_2->user_email, $subject, $body);
...
function add_product_to_cart() {
  if ( ! is_admin() ) {
    ...
    $product_id = $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key='_sku' AND meta_value='%s' LIMIT 1", 'k-1' ) );
...
{
$availability['availability'] = str_ireplace('Membership Not Available', 'Sold', $availability['availability']);
return $availability;
}
