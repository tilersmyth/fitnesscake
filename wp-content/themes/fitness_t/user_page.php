...
// $product_id = get_product_by_sku('k-1'); $current_user = wp_get_current_user();  $active_membership = get_post_meta($product_id->id, '_stock_status');
// if( !wc_customer_bought_product( $current_user->email, $current_user->ID, $product_id->id ) && ($active_membership[0] == 'instock') ){
      ...
      <div class="row">
        <div class="col-md-8">
          ...
          <div class="alert alert-success welcome-alert">
            ...
            <?php echo $current_user->first_name; ?>, Welcome to <?php echo get_bloginfo('name'); ?>! Below are routines that are available to you! Also, you can view individual exercises by selecting 'Exercises' above.
          </div>
          <?php endif; ?>
              ...
              <div class="recent-blogs">
                  <div class="media backend_sidebar_media">
                    ...
                    if (( 'exercises_single' == get_post_type($comment->comment_post_ID) ) || ($intended2[0])): ?>
                     <?php $user=get_user_meta($comment->user_id);?>
                      <h5 class="media-heading"><a class="panel_ellip" href="<?php echo get_permalink($comment->comment_post_ID); ?>"><?php echo $user["first_name"][0] .' '. $user["last_name"][0]  . ': ' . $comment->comment_content; ?></a></h5>
                      <div class="sidebar_meta"> 
