...
class WC_Subscriptions_Order {
	...
	public static function init() {
		...
		add_action( 'woocommerce_ajax_order_item', __CLASS__ . '::prefill_order_item_meta', 10, 2 ); // WC 2.0+
	...
	public static function get_sign_up_fee( $order, $product_id = '' ) {
		...
		elseif ( $recurring_total != $initial_payment )
			$sign_up_fee = max( $initial_payment - $recurring_total, 0 );
		else
			$sign_up_fee = 0;
	...
	public static function add_contains_subscription_hidden_field( $column ) {
		...
		if ( $column == 'order_status' ) {
			$contains_subscription = WC_Subscriptions_Order::order_contains_subscription( $post->ID ) ? 'true' : 'false';
...
<div id="recurring_order_totals"<?php if ( ! $contains_subscription ) echo $display_none; ?>>
	...
	<div class="totals_group">
		<h4><?php _e( 'Recurring Discounts', 'woocommerce-subscriptions'); ?> <a class="tips" data-tip="<?php _e( 'The discounts applied to each recurring payment charged in the future.', 'woocommerce-subscriptions' ); ?>" href="#">[?]</a></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Cart Discount:', 'woocommerce-subscriptions' ); ?></label>
			...
			<li class="right">
				<label><?php _e( 'Order Discount:', 'woocommerce-subscriptions' ); ?></label>
	...
	<div class="totals_group">
		<h4><?php _e( 'Shipping for Renewal Orders', 'woocommerce-subscriptions' ); ?></h4>
		<ul class="totals">
			<li class="wide">
			...
			<li class="left">
				...
				<input type="number" step="any" min="0" id="_order_recurring_shipping_total" name="_order_recurring_shipping_total" placeholder="0.00 <?php _e( '(ex. tax)', 'woocommerce-subscriptions' ); ?>" value="<?php echo self::get_recurring_shipping_total( $order ); ?>" class="first" />
			</li>
	...
	<div class="totals_group tax_rows_group">
		<h4><?php _e( 'Recurring Tax Rows', 'woocommerce-subscriptions' ); ?> <a class="tips" data-tip="<?php _e( 'These rows contain taxes included in each recurring amount for this subscription. This allows you to display multiple or compound taxes rather than a single total on future subscription renewal orders.', 'woocommerce-subscriptions' ); ?>" href="#">[?]</a></h4>
			...
			<?php
				...
				if ( is_array( $taxes ) && sizeof( $taxes ) > 0 ) :
					...
					foreach ( $taxes as $item_id => $item ) : ?>
						...
						<div class="tax_row">
							<p class="first">
								<label><?php _e( 'Recurring Tax Label:', 'woocommerce-subscriptions' ); ?></label>
							...
							<p class="last">
								<label><?php _e( 'Compound:', 'woocommerce-subscriptions' ); ?>
								<input type="checkbox" name="_order_recurring_taxes[<?php echo $loop; ?>][compound]" <?php checked( $item['compound'], 1 ); ?> /></label>
							</p>
							<p class="first">
								<label><?php _e( 'Recurring Cart Tax:', 'woocommerce-subscriptions' ); ?></label>
	...
	<div class="totals_group">
		<h4><?php _e( 'Recurring Totals', 'woocommerce-subscriptions' ); ?> <a class="tips" data-tip="<?php _e( 'The total amounts charged for each future recurring payment.', 'woocommerce-subscriptions' ); ?>" href="#">[?]</a></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Recurring Order Total:', 'woocommerce-subscriptions' ); ?></label>
			...
			<li class="right" style="font-size: 1em;">
				<div style="font-weight: normal; color: #333;"><?php _e( 'Payment Method:', 'woocommerce-subscriptions' ); ?></div>
				<div style="padding-top: 7px; font-style: italic;"><?php echo ( $manual_renewal ) ? __( 'Manual', 'woocommerce-subscriptions' ) : $order->recurring_payment_method_title; ?></div>
			</li>
...
<?php
	...
	public static function add_sub_info_email( $order, $is_admin_email ) {
		...
		if ( get_option( WC_Subscriptions_Admin::$option_prefix . '_add_sub_info_email', 'yes' ) && self::order_contains_subscription( $order ) && ! $is_admin_email ) {
			global $woocommerce;

			$template_base  = plugin_dir_path( WC_Subscriptions::$plugin_file ) . 'templates/';
			$template = ( 'plain' == $woocommerce->woocommerce_email->emails['WC_Email_Customer_Completed_Order']->email_type ) ? 'emails/plain/subscription-info.php' : 'emails/subscription-info.php';
