...
class WC_Subscriptions_Admin {
	...
	public static function init() {
		...
		add_action( 'add_meta_boxes', __CLASS__ . '::add_related_orders_meta_box' );
	...
	public static function subscription_pricing_fields() {
		...
		woocommerce_wp_select( array(
			...
			'description' => sprintf( __( 'with a %s', 'woocommerce-subscriptions' ), get_woocommerce_currency_symbol() ),
			)
		);
	...
	public static function subscription_advanced_fields() {
		...
		woocommerce_wp_checkbox( array(
			...
			'cbvalue'     => 'yes',
			'description' => __( 'Only allow a customer to have one active subscription to this product.', 'woocommerce-subscriptions' ),
			)
		);
...
?>
	...
	<td class="sign-up-fee-cell show_if_variable-subscription">
		<label><?php printf( __( 'Sign-up Fee (%s)', 'woocommerce-subscriptions' ), get_woocommerce_currency_symbol() ) ?></label>
<?php
		// Sign-up Fee
		woocommerce_wp_text_input( array(
...
<?php
	...
	public static function variable_subscription_bulk_edit_actions() {
		...
		<option value="variable_subscription_period_interval" rel="select"><?php _e( 'Subscription billing interval', 'woocommerce-subscriptions' ); ?></option>
		<option value="variable_subscription_period" rel="select"><?php _e( 'Subscription period', 'woocommerce-subscriptions' ); ?></option>
		<option value="variable_subscription_length" rel="select"><?php _e( 'Subscription length', 'woocommerce-subscriptions' ); ?></option>
		<option value="variable_subscription_trial_length"><?php _e( 'Free trial length', 'woocommerce-subscriptions' ); ?></option>
		<option value="variable_subscription_trial_period" rel="select"><?php _e( 'Free trial period', 'woocommerce-subscriptions' ); ?></option>
		<?php endif;
	}
	...
	public static function enqueue_styles_scripts() {
		...
		$is_woocommerce_screen = ( in_array( $screen->id, array( 'product', 'edit-shop_order', 'shop_order', self::$admin_screen_id ) ) ) ? true : false;
		...
		if ( $is_woocommerce_screen ) {
			...
			$woocommerce_admin_script_handle = ( WC_Subscriptions::is_woocommerce_pre_2_1() ) ? 'woocommerce_writepanel' : 'woocommerce_admin_meta_boxes';
			...
			if( $screen->id == 'product' ) {
				...
				$script_params = array(
					...
					'changeMetaWarning' => __( "WARNING: Bad things are about to happen!\n\nThe payment gateway used to purchase this subscription does not support modifying a subscription's details.\n\nChanges to the billing period, recurring discount, recurring tax or recurring total may not be reflected in the amount charged for each recurring billing period.", 'woocommerce-subscriptions' ),
...
<div class="wrap">
	...
	public static function get_settings() {
		...
		if ( count( $available_gateways ) == 0 )
			$available_gateways_description = sprintf( __( 'No payment gateways capable of processing automatic subscription payments are enabled. Please enable the %sPayPal Standard%s gateway.', 'woocommerce-subscriptions' ), '<strong><a href="' . admin_url( 'admin.php?page=woocommerce&tab=payment_gateways#gateway-paypal' ) . '">', '</a></strong>' );
		...
		return apply_filters( 'woocommerce_subscription_settings', array(
			...
			array(
				'name'          => __( 'Allow Customer Suspensions', 'woocommerce-subscriptions' ),
...
<div id="message" class="updated woocommerce-message wc-connect woocommerce-subscriptions-activated">
	...
	public static function add_related_orders_meta_box() {
		...
		if ( 'shop_order' == $current_screen->id && ( WC_Subscriptions_Renewal_Order::is_renewal( $post_id, array( 'order_role' => 'child' ) ) || WC_Subscriptions_Order::order_contains_subscription( $post_id ) ) ) {
			add_meta_box(
				...
				__( 'Related Subscription Orders', 'woocommerce-subscriptions' ),
				__CLASS__ . '::related_orders_meta_box',
				'shop_order'
			);
		}
	}
	...
	public static function add_system_status_items( $debug_data ) {
		...
		$debug_data['wcs_debug'] = array(
			...
			'note'    => ( $is_wcs_debug ) ? __( 'Subscriptions debug mode is enabled.', 'woocommerce-subscriptions' ) :  __( 'Subscriptions debug mode is disabled.', 'woocommerce-subscriptions' ),
			'success' => $is_wcs_debug ? 0 : 1,
		);

		$debug_data['wcs_staging'] = array(
			'name'    => __( 'Subscriptions Staging Mode', 'woocommerce-subscriptions' ),
			'note'    => ( WC_Subscriptions::is_duplicate_site() ) ? __( 'WooCommerce Subscriptions is in <strong>staging</strong> mode.', 'woocommerce-subscriptions' ) :  __( 'WooCommerce Subscriptions is in <strong>live</strong> mode.', 'woocommerce-subscriptions' ),
