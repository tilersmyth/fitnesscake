...
class WC_Subscriptions_Switcher {
	...
	public static function init(){
		...
		add_action( 'woocommerce_order_status_pending_to_processing', __CLASS__ . '::maybe_complete_switch', 10, 1 );
		add_action( 'woocommerce_order_status_pending_to_completed', __CLASS__ . '::maybe_complete_switch', 10, 1 );
		...
		// Make sure the first payment date a new subscription uses is correct
		add_filter( 'woocommerce_subscriptions_calculated_next_payment_date', __CLASS__ . '::calculate_first_payment_date', 11, 4 );
	...
	public static function subscription_switch_handler() {
		...
		if ( isset ( $_GET['switch-subscription'] ) ) {
			...
			if ( ! WC_Subscriptions_Manager::user_owns_subscription( $_GET['switch-subscription'] ) ) {
				...
				WC_Subscriptions::add_notice( __( 'Choose a new subscription.', 'woocommerce-subscriptions' ), 'success' );

			}

		} elseif ( ( is_cart() || is_checkout() ) && false !== self::cart_contains_subscription_switch() ) {

			WC_Subscriptions::add_notice( __( 'Once sign up is complete, this will replace your existing subscription.', 'woocommerce-subscriptions' ), 'success' );
	...
	public static function add_settings( $settings ) {
		...
		array_splice( $settings, 12, 0, array(
			...
			array(
				'name'    => __( 'Apportion Recurring Payments', 'woocommerce-subscriptions' ),
				'desc'    => __( 'When switching to a subscription with a different recurring payment or billing period, should the price paid for the existing billing period be apportioned when switching to the new subscription?', 'woocommerce-subscriptions' ),
				'tip'     => '',
			...
			array(
				'name'    => __( 'Apportion Sign up Fee', 'woocommerce-subscriptions' ),
			...
			array(
				'name'    => __( 'Apportion Subscription Length', 'woocommerce-subscriptions' ),
	...
	public static function get_switch_link( $subscription_key ) {
		...
		if ( 0 !== $product->post->post_parent ) // Grouped product
	...
	public static function maybe_set_apporitioned_totals( $total ) {
		...
		foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( 'yes' == $apportion_recurring_price || ( 'virtual' == $apportion_recurring_price && $is_virtual_product ) ) {
				...
				} elseif ( $old_price_per_day > $new_price_per_day ) {
	 ...
	 * free trial for subscription switches) and also if the length is being apportioned, don't display the length until
	 * checkout.
	 *
	 * @since 1.4
	 */
